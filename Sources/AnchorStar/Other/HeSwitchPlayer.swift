...
class HeSwitchPlayer: UIView, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    override init(frame: CGRect) {
        ...
        hIsTranslator.font = textApprovalCustomTitle(16)
        ...
        zTempStore.configuration?.baseBackgroundColor = .clear
        ...
        dAttributeClass.snp.makeConstraints { make in
            make.bottom.leading.trailing.equalTo(zExtendsType)
        }
        parseResizeMode()
    ...
    @objc func requestThreadId() {
        if rMockAudio == true {
            qGetAnimation.backgroundColor = showAllEditors(roundPriceChange("color_alturas"), 0.6)
        }else {
